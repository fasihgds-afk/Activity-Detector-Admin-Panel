...
function useColors(theme) {
  ...
  const okBg = alpha(theme.palette.success.main, theme.palette.mode === "dark" ? 0.22 : 0.1);
  ...
  const progressTrack =
    theme.palette.mode === "dark" ? alpha(theme.palette.common.white, 0.16) : alpha(theme.palette.text.primary, 0.1);
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    pollRef.current = setInterval(fetchAll, 60000); // 60s
  ...
  function sessionsInScope(list) {
    const arr = Array.isArray(list) ? list : [];
    if (scope === "today") return arr.filter((s) => ymdOf(s) === todayYmd);
    return arr.filter((s) => {
      const sd = ymdOf(s);
  ...
  const leaderboard = useMemo(() => {
    ...
    for (const e of employees) {
      ...
      if (genEx > 0 || namEx > 0) {
        status = "Exceeded";
        color = "error";
      } else if ((genCap && general / genCap >= 0.8) || (namCap && namaz / namCap >= 0.8)) {
        status = "Near Limit";
        color = "warning";
      }
      ...
      didParseCell: (d) => {
        if (d.section === "body") {
          ...
          d.cell.styles.fillColor =
            row.status === "Exceeded" ? [254, 226, 226] : row.status === "Near Limit" ? [255, 251, 235] : [236, 253, 245];
  ...
  function downloadXls(filename, headers, rows) {
    const headerHtml = headers
      .map(
        (h) =>
          `<th style="background:#111827;color:#fff;padding:10px 8px;border:1px solid #e5e7eb;text-align:center;font-weight:700">${h}</th>`
      )
      .join("");
    const rowHtml = rows
      .map(
          ...
          {loading ? (
            ...
            <Card
              sx={{
                ...
                color: theme.palette.getContrastText(C.primary),
              }}
            >
              <CardContent>
                ...
                <Button
                  variant="outlined"
                  sx={{ mt: 1, color: "inherit", borderColor: alpha("#fff", 0.8) }}
        ...
        <ResponsiveContainer width="100%" height={300}>
          <PieChart>
            <Pie
              ...
              label
            >
              {[C.general, C.namaz, C.official].map((c, i) => (
                <Cell key={i} fill={c} />
              ))}
            </Pie>
            <RTooltip />
