...
export default function Login() {
  ...
  async function handleLogin(e) {
    ...
    try {
      ...
      const { data } = await axios.post(`${API}/auth/login`, body, { timeout: 15000 });
      ...
      setAlert({ open: true, type: "error", msg: "❌ " + (err?.response?.data?.error || err.message) });
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <Box sx={{ height: "100vh", display: "flex", alignItems: "center", justifyContent: "center",
      ...
      <Paper elevation={12} sx={{ p: 6, width: 420, borderRadius: "20px", textAlign: "center",
        ...
        <Typography variant="h5" fontWeight="bold" gutterBottom>Employee Monitor — Login</Typography>

        <ToggleButtonGroup color="primary" exclusive value={mode} onChange={(_, v) => v && setMode(v)} sx={{ my: 2 }}>
      ...
      <Snackbar open={alert.open} autoHideDuration={2000} onClose={() => setAlert({ ...alert, open: false })}
        ...
        <Alert severity={alert.type} sx={{ width: "100%", fontWeight: "bold" }}>{alert.msg}</Alert>
      </Snackbar>
    </Box>
  );
}
